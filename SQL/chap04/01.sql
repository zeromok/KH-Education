...
-- 가장 원시적인 조인, 유의미한 데이터는 없다.
-- 이 무의미한 조인으로 부터 유의미한 데이터를 뽑아내는게 동등 조인
-- 조인의 근간
--  가. 유효한 데이터로 사용되지 못함.
--  나. 조인조건이 생략된 경우에 발생.
--
-- * 조인결과: 테이블1 x ... x 테이블n 개의 레코드 생성
-- ex) 테이블1(카디널리티10) + 테이블2(카디널리티5) = 결과셋 = (카디널리티50)
-- ------------------------------------------------------
-- Basic Syntax)
...
FROM
    ...
    departments dept
WHERE
    emp.department_id = dept.department_id  -- 조인조건(식)
    AND last_name='Whalen';                 -- 검색조건(식)
